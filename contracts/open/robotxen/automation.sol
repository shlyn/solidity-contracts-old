...
contract AutomationRobot is AutomationStorage, transferHelper {
    ...
    function claim(uint256 mintId, uint256 batchId) external {
        ...
        require(result.valueLeft >= info.maxGasConsumedPerBatchClaim, "task done");
...
contract AutomationMember is AutomationStorage, transferHelper {
    ...
    function startMintTask(uint256 term, uint256 maxGasPrice) external payable {
        ...
        require(
            mintTasks[msg.sender].length - stoppedMintTasks[msg.sender].length < taskCountPerMember,
            "mint task count exceeds limit"
        );

        uint256 maxGasConsumedPerBatchMint = maxGasPrice * GAS_USED_PER_BATCH_MINT;
     ...
     * @param maxGasPrice The max gas price you can accept when minting xen
     */
    function startClaimTask(uint256 mintId, uint256 maxGasPrice) external payable {
        require(maxGasPrice >= minGasPrice, "max gas price is too small");
    ...
    function setClaimMaxGasPrice(uint256 mintId, uint256 maxGasPrice) external {
        ...
        info.maxGasPrice = maxGasPrice;
        info.maxGasConsumedPerBatchClaim = maxGasPrice * GAS_USED_PER_BATCH_CLAIM;
    ...
    event StopClaimTask(uint256 indexed mintId, uint256 claimId, uint256 valueLeft);
    event TopUpClaimTask(uint256 indexed mintId, uint256 value);
    event SetClaimMaxGasPrice(uint256 indexed mintId, uint256 claimId, uint256 maxGsPrice);
}
