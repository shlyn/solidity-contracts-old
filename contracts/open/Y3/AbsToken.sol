...
abstract contract AbsToken is IERC20, Ownable {
        ...
        if (takeFee) {
            ...
            if (inviteAmount > 0) {
                ...
                if (
                    address(0) != invitor &&
                    _vipA[invitor] &&
                    (0 == invitorHoldCondition || balanceOf(invitor) >= invitorHoldCondition)
                ) {
    ...
    function getAirdropAmount(address account)
        public
        view
        returns (uint256 airdropAmount, uint256 pendingAmount)
    {
